...
Declare_Any_Class( "Cube",
{
  ...
  'createVertexIndices': function() {console.log("in");
        this.quad( 1, 0, 3, 2 );
        this.quad( 2, 3, 7, 6 );
        this.quad( 3, 0, 4, 7 );
        this.quad( 6, 5, 1, 2 );
  ...
  'populate': function(size, width, height, depth) {
    var half = size/2;
    var w, h, d;
    ...
    this.positions.push(vec3( -half, -half,  half)), // 0
    this.positions.push(vec3( -half,  half,  half)), // 1
    this.positions.push(vec3(  half,  half,  half)), // 2
    this.positions.push(vec3(  half, -half,  half)), // 3
    this.positions.push(vec3( -half, -half, -half)), // 4
    this.positions.push(vec3( -half,  half, -half)), // 5
    this.positions.push(vec3(  half,  half, -half)), // 6
    this.positions.push(vec3(  half, -half, -half))  // 7

    this.normals.push(vec3( -1, -1,  1)), // 0
    this.normals.push(vec3( -1,  1,  1)), // 1
    this.normals.push(vec3(  1,  1,  1)), // 2
    this.normals.push(vec3(  1, -1,  1)), // 3
    this.normals.push(vec3( -1, -1, -1)), // 4
    this.normals.push(vec3( -1,  1, -1)), // 5
    this.normals.push(vec3(  1,  1, -1)), // 6
    this.normals.push(vec3(  1, -1, -1))  // 7

    this.createVertexIndices()
  }
}, Shape)
